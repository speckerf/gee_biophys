...
def load_SL2P_model(variable="lai", rel_path="."):

    folder_name = "models/sl2p_params"
    estimate_json = os.path.join(
        rel_path, folder_name, f"{variable}-estimation_SL2P_Corrected_LeafToolBox.json"
    )
    uncertainty_json = os.path.join(
        rel_path, folder_name, f"{variable}-uncertainty_SL2P_Corrected_LeafToolBox.json"
    )
